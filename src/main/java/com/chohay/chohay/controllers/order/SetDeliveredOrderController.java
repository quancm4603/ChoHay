 ...
 * @author caomi
 */
@WebServlet(name="SetDeliveredOrderController", urlPatterns={"/set-delivered-order"})
public class SetDeliveredOrderController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            if (userId == orderService.getOrderById(orderId).getCustomerId()) {
                ...
                session.setAttribute("cancelStatus", "Xác nhận thành công, đơn hàng đã chuyển về mục ĐÃ GIAO");
            }
        } catch (SQLException ex) {
            session.setAttribute("cancelStatus", "Xác nhận thất bại");
