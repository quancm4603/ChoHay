...
//                    + "}\n"; // Thay thế {...} bằng nội dung JSON thích hợp
    ...
    public String uploadImageFromInputStream(InputStream inputStream, String fileName, String contentType) throws IOException {
        // Khởi tạo FirebaseOptions từ chuỗi JSON
        byte[] jsonBytes = jsonContent.getBytes(StandardCharsets.UTF_8);
        GoogleCredentials credentials = GoogleCredentials.fromStream(new ByteArrayInputStream(jsonBytes));
        Storage storage = StorageOptions.newBuilder().setCredentials(credentials).build().getService();
        Bucket bucket = storage.get(bucketName);

        // Tạo đối tượng BlobInfo để chỉ định tên file và loại MIME (content type)
        BlobId blobId = BlobId.of(bucketName, fileName);
        BlobInfo blobInfo = BlobInfo.newBuilder(blobId)
                .setContentType(contentType) // Sử dụng loại MIME được truyền vào
                .build();

        // Đọc dữ liệu từ InputStream và tải lên Firebase Storage với thông tin tên file và loại MIME
